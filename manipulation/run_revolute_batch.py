...
OUTPUT_ROOT = MANIP / "output" / "revolute_tests"
SUMMARY = DATASET / "mixedcabinet_movable_parts_summary.json"
OUT = DATASET / "mixedcabinet_revolute_open_test_results.json"


def select_targets(summary):
    targets = []
    for asset_id, asset in sorted(summary["assets"].items()):
        if asset.get("revolute_count", 0) <= 0:
            continue
        parts = sorted(
            asset.get("parts", []),
            key=lambda part: (part.get("part_id", 10**9), part.get("joint_order", 10**9)),
        )
        revolute_parts = [part for part in parts if part.get("joint_type") == "revolute"]
        if not revolute_parts:
            continue
        target = dict(revolute_parts[0])
        target.update(
            {
                "asset_id": asset_id,
        ...
        if asset.get("revolute_count", 0) == 1 and asset.get("prismatic_count", 0) == 0:
            target["selection_reason"] = "single_revolute"
        elif asset.get("revolute_count", 0) >= 1 and asset.get("prismatic_count", 0) >= 1:
            target["selection_reason"] = "mixed_sorted_first_revolute"
        else:
            target["selection_reason"] = "revolute_sorted_first"
        targets.append(target)
    return targets


def load_result(target):
    result_path = OUTPUT_ROOT / f"partnet_mobility_mixedcabinet_{target['asset_id']}" / "result.json"
...
def main():
    ...
    print(f"[BATCH] selected {len(targets)} revolute assets", flush=True)
    results = []
